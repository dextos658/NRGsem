// #part /glsl/shaders/renderers/EAMuMC/integrate/vertex
...
// #part /glsl/shaders/renderers/EAMuMC/integrate/fragment
...
void main() {
    ...
    for (uint i = 0u; i < uSteps; i++) {
        ...
        if (any(greaterThan(photon.position, vec3(1))) || any(lessThan(photon.position, vec3(0)))) {
            // out of bounds: add current radiance to the total radiance, normalize it, increase photon’s samples and reset it
            //vec4 envSample = sampleEnvironmentMap(photon.direction);
            vec3 radiance = (0, 0, 0); //* envSample.rgb;
            photon.samples++;
            photon.radiance += (radiance - photon.radiance) / float(photon.samples);
            resetPhoton(r, photon);
        }
        // else if (photon.bounces >= uMaxBounces) { // not used
        ...
        //}
            ...
            // absorption; lower transmittance and radiance
            vec3 radiance = envSample.rgb;
            float weightA = muAbsorption / (uMajorant * PAbsorption);
            photon.transmittance *= 1.0 - weightA;
        } else if (r.y < PAbsorption + PEmission) {
            // scattering; NOW became emission; raise transmittance and radiance
            r = rand(r);
            float weightS = muEmission / (uMajorant * PEmission);
            photon.transmittance *= volumeSample.rgb * weightS;
            // photon.direction = sampleHenyeyGreenstein(uScatteringBias, r, photon.direction); // ali je direction pri emisiji sploh pomemben???
            // photon.bounces++; // po napotkih iz vaj, tega ne stejemo
        } else {
            // null collision: hits fictitious material
    ...
    oTransmittance = vec4(photon.transmittance, photon.transmittance.a);
    oRadiance = vec4(photon.radiance, float(photon.samples));
}

// #part /glsl/shaders/renderers/EAMuMC/render/vertex
...
// #part /glsl/shaders/renderers/EAMuMC/render/fragment
...
// #part /glsl/shaders/renderers/EAMuMC/reset/vertex
...
// #part /glsl/shaders/renderers/EAMuMC/reset/fragment
