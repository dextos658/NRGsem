// #part /glsl/shaders/renderers/EAMMC/integrate/vertex
...
// #part /glsl/shaders/renderers/EAMMC/integrate/fragment
...
void main() {
    ...
    for (uint i = 0u; i < uSteps; i++) {
        r = rand(r);
        float t = -log(r.x) / uAbsorptionCoefficient;
        ...
        // float PNull = abs(muNull) / uMajorant;
        float PAbsorption = muAbsorption / uAbsorptionCoefficient;
        // float PEmission = muEmission / muMajorant;

        if (any(greaterThan(photon.position, vec3(1))) || any(lessThan(photon.position, vec3(0)))) {
            // out of bounds; black background
            // vec4 envSample = sampleEnvironmentMap(photon.direction);
            vec3 radiance = vec3(0); // * envSample.rgb;
            photon.samples++;
            ...
            // absorption;
            vec3 radiance = volumeSample.rgb;
            photon.samples++;
            photon.radiance += (radiance - photon.radiance) / float(photon.samples);
            resetPhoton(r, photon);

        }
        // else if (r.y < PAbsorption // + PEmission) { // not in operation
            // scattering;
            // r = rand(r);
...
// #part /glsl/shaders/renderers/EAMMC/render/vertex
...
// #part /glsl/shaders/renderers/EAMMC/render/fragment
...
// #part /glsl/shaders/renderers/EAMMC/reset/vertex
...
// #part /glsl/shaders/renderers/EAMMC/reset/fragment
